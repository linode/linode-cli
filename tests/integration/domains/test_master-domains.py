...
from tests.integration.helpers import delete_all_domains, exec_test_command, delete_tag, FAILED_STATUS_CODE, SUCCESS_STATUS_CODE
...
def test_create_master_domain():
    ...
    result = exec_test_command(BASE_CMD+['create', '--type', 'master', '--domain', 'BC'+timestamp+'-example.com',
                                         '--soa_email=pthiel'+timestamp+'@linode.com', '--text', '--no-header',
                                         '--delimiter', ",", '--format=id,domain,type,status,soa_email']).stdout.decode()
