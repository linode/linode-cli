...
DEFAULT_RANDOM_PASS = exec_test_command(['openssl', 'rand', '-base64', '32']).stdout.decode().rstrip()
DEFAULT_REGION = 'us-east'
DEFAULT_TEST_IMAGE = os.popen('linode-cli images list --format id --text --no-header | egrep "linode\/.*" | head -n 1').read().rstrip()
DEFAULT_LINODE_TYPE = os.popen("linode-cli linodes types --text --no-headers --format='id' | xargs | awk '{ print $1 }'").read().rstrip()
...
def wait_until(linode_id: "str", timeout, status: "str", period=5):
    ...
    while time.time() < must_end:
        result = exec_test_command(['linode-cli', 'linodes', 'view', linode_id, '--format', 'status', '--text', '--no-headers']).stdout.decode()
        if status in result:
            return True
        time.sleep(period)
    return False
...
def shutdown_linodes():
    linode_ids = os.popen('linode-cli --text --no-headers linodes list --format "id" | grep -v "linuke-keep" | awk "{ print $1 }" | xargs').read().split()
...
def create_linode_and_wait(test_plan=DEFAULT_LINODE_TYPE, test_image=DEFAULT_TEST_IMAGE, ssh_key=""):
