...
def test_create_linode_with_backup_disabled():
    ...
    result = exec_test_command(BASE_CMD+['list', '--format=id,enabled', '--delimiter', ',', '--text', '--no-headers']).stdout.decode()
...
def test_create_backup_with_backup_enabled():
    ...
    linode_id = exec_test_command(
        ['linode-cli', 'linodes', 'create', '--backups_enabled', 'true', '--type', linode_type, '--region', DEFAULT_REGION, '--image', DEFAULT_TEST_IMAGE,
         '--root_pass', DEFAULT_RANDOM_PASS, '--text', '--no-headers', '--format=id']).stdout.decode().rstrip()
...
def test_take_snapshot_of_linode():
    ...
    result = exec_test_command(BASE_CMD + ['snapshot', linode_id, '--label', snapshot_label, '--text', '--delimiter', ',', '--no-headers']).stdout.decode()
    assert(re.search("[0-9]+,pending,snapshot,[0-9]+-[0-9]+-[0-9]+T[0-9]+:[0-9]+:[0-9]+,"+snapshot_label, result))
...
def test_view_the_snapshot():
    ...
    result = exec_test_command(BASE_CMD + ['snapshot', linode_id, '--label', new_snapshot_label, '--text', '--delimiter', ',', '--no-headers']).stdout.decode()
    assert(re.search("[0-9]+,pending,snapshot,[0-9]+-[0-9]+-[0-9]+T[0-9]+:[0-9]+:[0-9]+,"+new_snapshot_label, result))

    result = exec_test_command(BASE_CMD+['backups-list', linode_id, '--delimiter', ',', '--text', '--no-headers']).stdout.decode()
...
def test_cancel_backups():
    ...
    # cancel snapshot
    exec_test_command(BASE_CMD + ['backups-cancel', linode_id, '--text', '--no-headers'])
