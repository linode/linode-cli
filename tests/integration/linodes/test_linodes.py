...
def test_create_linodes_with_a_label():
    result = exec_test_command(BASE_CMD + ['create', '--type', 'g6-standard-2', '--region',  'us-east', '--image',
                                          DEFAULT_TEST_IMAGE, '--label', 'cli-1', '--root_pass', DEFAULT_RANDOM_PASS,
                                          '--text', '--delimiter', ",", '--no-headers', '--format',
...
def test_view_linode_configuration(setup_linodes):
    ...
    assert(re.search(linode_id+","+DEFAULT_LABEL+",us-east,g6-standard-2,"+DEFAULT_TEST_IMAGE, result))


def test_create_linode_with_min_required_props():
    result = exec_test_command(BASE_CMD+['create', '--type', 'g6-standard-2', '--region', 'us-east', '--root_pass',
                                         DEFAULT_RANDOM_PASS, '--no-defaults', '--text', '--delimiter', ",", '--no-headers',
                                         '--format', 'id,region,type']).stdout.decode()
    assert(re.search("[0-9]+,us-east,g6-standard-2", result))


def test_create_linodes_fails_without_a_root_pass():
    result = exec_failing_test_command(BASE_CMD+['create', '--type', 'g6-standard-2', '--region', 'us-east', '--image',
                                                 DEFAULT_TEST_IMAGE, '--text', '--no-headers']).stderr.decode()
    assert('Request failed: 400' in result)
    assert('root_pass	root_pass is required' in result)


def test_create_linode_without_image_and_not_boot():
    linode_type = os.popen("linode-cli linodes types --text --no-headers --format=id | xargs | awk '{ print $1 }'").read().rstrip()
    linode_region = os.popen("linode-cli regions list --format=id  --text --no-headers | xargs | awk '{ print $1 }'").read().rstrip()

    exec_test_command(BASE_CMD+['create', '--no-defaults', '--label', 'cli-2', '--type', linode_type, '--region', linode_region, '--root_pass', DEFAULT_RANDOM_PASS]).stdout.decode()

    linode_id = exec_test_command(BASE_CMD + ['list', '--label', 'cli-2', '--text', '--no-headers',
                                              '--format', 'id']).stdout.decode().rstrip()

    wait_until(linode_id=linode_id, timeout=180, status="offline")

    result = exec_test_command(BASE_CMD+['view', linode_id, '--format', 'status', '--text', '--no-headers']).stdout.decode()
